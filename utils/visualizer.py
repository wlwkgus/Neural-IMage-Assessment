...
class Visualizer():
    ...
    def plot_current_nums(self, epoch, counter_ratio, nums_dict, display_id=None, title=None):
        if display_id is None:
            display_id = self.display_id
        if title is None:
            title = self.name + ' loss over time'
        if not hasattr(self, 'plot_data'):
            self.plot_data = {'X': [], 'Y': [], 'legend': list(nums_dict.keys())}
        self.plot_data['X'].append(epoch + counter_ratio)
        self.plot_data['Y'].append([nums_dict[k] for k in self.plot_data['legend']])
        self.vis.line(
